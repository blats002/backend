...
public class JeePasswordResetServerResource extends BaseServerResource
  ...
  private static final Logger LOG = LoggerFactory.getLogger(JeePasswordResetServerResource.class);
          ...
          if (beforeSave(
            ...
            Boolean success =
                userRepository.updateUserPassword(
                    appId, namespace, defaultUserStore, userEntity.getEntityId(), newHashPassword);
            if (success) {
              afterSave(
                  ComparableMapBuilder.newBuilder()
                      .put("entityId", entityId)
      ...
      if (validate(entity.getUsername(), entity.getNewPassword())) {
        if (entity.getPassword() != null && !entity.getPassword().isEmpty()) {
          ...
          if (userEntity == null) {
            ...
            String usernameWebToken =
                webTokenService.createToken(getApp().getMasterKey(), username);

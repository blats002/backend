...
public class JeeRolesServerReource extends BaseServerResource
    ...
    public Roles getRoles() {
        ...
        if (!isMaster()) {
            ...
            List<Role> results = roleRepository.listRoles(appId, namespace, defaultRoleStore, authUserId,
            ...
            List<Role> results = roleRepository.listRoles(appId, namespace, defaultRoleStore, null,
    ...
    public Role createRole(Role entity) {
        try {
            ...
            if(beforeSave(ComparableMapBuilder.newBuilder().put("name", roleName).build(), appId, entityType)) {
                roleId = roleRepository.createRole(appId, namespace, defaultRoleStore, roleName, read, write, publicRead, publicWrite, actions);
                if(roleId != null) {

...
public class LogoutMethod implements CustomCodeMethod {
	...
	public CustomCodeResponse execute(CustomCodeRequest request) {
		...
		int status = -1;
		try {
			HttpResponse<String> postRequest = Unirest.post(Config.PARSE_URL + "/logout")
					...
					.asString();
			String getBody = postRequest.getBody();
